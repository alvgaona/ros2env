...
---
import { Tabs, TabItem, Aside, Steps, Badge, LinkButton, FileTree } from '@astrojs/starlight/components';
...
- **🔗 Symlink Management** - Automatically creates and maintains symlinks in `/opt/ros/` for full compatibility with ROS 2 tools and tutorials
- **🔄 Easy Switching** - Switch between ROS 2 distributions instantly with automatic environment variable management
- **🛡️ Environment Isolation** - Clean separation between distributions prevents conflicts and ensures reproducible builds
- **🔍 Auto-Detection** - Automatically discovers ROS 2 installations in `~/.pixi/envs/` without manual configuration
